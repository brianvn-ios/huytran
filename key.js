(function(){
  // ===== KẾT NỐI FIREBASE GIỐNG FILE SERVER KEY =====
  const FIREBASE_DB_URL = 'https://serverkey-e7cc6-default-rtdb.firebaseio.com';
  const BRAND_TITLE = 'PANEL API SERVER KEY';
  ...
  // ===== VERIFY KEY (chỉ kiểm tra, không gắn thiết bị) =====
  async function verifyKey(key){
    const data = await fbGet(`keys/${key}`);
    if(!data) return { ok:false, error:'NOT_FOUND' };
    if(data.active === false) return { ok:false, error:'REVOKED' };
    if(Date.now() > data.expire) return { ok:false, error:'EXPIRED' };
    return { ok:true, data:{ expiresAt: data.expire, days: data.days, maxDevices: data.maxDevices, devices: data.devices||{} } };
  }

  // ===== ACTIVATE KEY (gắn thiết bị vào key) =====
  async function activateKey(key, deviceId){
    ...
    // Nếu thiết bị đã gắn trước đó thì cho qua
    if(devices[deviceId]){
      return { ok:true, data:{ expiresAt: data.expire, days: data.days, maxDevices: data.maxDevices, deviceId } };
    }

    // Kiểm tra số thiết bị tối đa
    if(deviceList.length >= data.maxDevices){
      return { ok:false, error:'BOUND_TO_ANOTHER_DEVICE' };
    }

    // Gắn thiết bị mới
    await fbUpdate(`keys/${key}/devices`, { [deviceId]: Date.now() });
    return { ok:true, data:{ expiresAt: data.expire, days: data.days, maxDevices: data.maxDevices, deviceId } };
  }
  ...
  function build(){
      ...
      <div class="vg-panel">
        ...
        <div class="vg-bd">
          <div>
            ...
            <div class="vg-field">
              <input id="vgKey" class="vg-input" type="text" placeholder="DÁN APP-XXXX-XXXX" autocomplete="one-time-code" inputmode="latin">
    ...
    $('#vgContact').onclick  = ()=> window.open('https://zalo.me/0589198977', '_blank');
  ...
  async function onCheck(){
    ...
    if(j.ok){
      ...
      const d = j.data;
      updateStatus(d);
      setMsg('ok', `Mã hợp lệ<br>📅 ${d.days} ngày &nbsp;|&nbsp; 📱 ${d.maxDevices} thiết bị<br>Hết hạn: <b>${fmt(d.expiresAt)}</b>`, j);
    }else{
  ...
  async function onActivate(){
    ...
    if(j.ok){
      ...
      const map={
        BOUND_TO_ANOTHER_DEVICE:'Mã đã đạt giới hạn thiết bị, không thể kích hoạt thêm.',
  ...
  async function guardOnLoad(){
    ...
    const v = await verifyKey(savedKey);
    if(!v.ok){ show(); return; }
    // Kiểm tra thiết bị đã được gắn với key chưa
    const devices = v.data.devices || {};
    if(!devices[deviceId]){ show(); return; }
    updateStatus(v.data);
